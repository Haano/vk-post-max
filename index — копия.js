...
vk.updates.on("wall_post_new", async (ctx) => {
  ...
  if (post.post_type === "suggest") {
    console.log("⏭ Пропущен предложенный пост");
    return; // Пропускаем предложенные посты
  }

  // Также можно проверять другие типы
  if (post.post_type !== "post") {
    console.log(`⏭ Пропущен пост типа: ${post.post_type}`);
    return;
  }
